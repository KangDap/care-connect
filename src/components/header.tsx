...
const BellIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#193C1F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export const Header = ({ withSearch = true, withLogo = false, onProfileClick }: any) => {
  ...
  const [isLoggingOut, setIsLoggingOut] = useState(false); // State baru agar sama dengan layout
  const [searchQuery, setSearchQuery] = useState('');
  const dropdownRef = useRef<HTMLDivElement>(null);

  const profileName = session?.user?.name || 'User';
  const profileId = session?.user?.id ? `#${session.user.id.slice(-5).toUpperCase()}` : '-';
  ...
  // Logika handleLogout disamakan persis dengan DashboardLayout
    ...
    <>
      <header className="h-[90px] border-b border-[#D0D5CB] flex items-center justify-between px-12 bg-[#F7F3ED]/80 backdrop-blur-md shrink-0 relative z-[100]">
        ...
        <div className="flex items-center gap-8 flex-grow">
          {withLogo && <div className="shrink-0"><Logo /></div>}
          ...
          <div className="relative" ref={dropdownRef}>
            <div onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-4 pl-6 border-l border-[#D0D5CB] cursor-pointer group select-none">
              <div className="text-right hidden md:block">
                <p className="text-[15px] font-bold text-[#193C1F] leading-tight">{profileName}</p>
                <p className="text-[11px] text-[#8EA087] font-bold uppercase mt-0.5">ID: {profileId}</p>
              </div>
              <div className={`w-12 h-12 rounded-2xl overflow-hidden border-2 shadow-md transition-all ${isProfileOpen ? 'border-[#8EA087] ring-4 ring-[#8EA087]/10' : 'border-white group-hover:border-[#8EA087]'}`}>
                <Image src={profileAvatar} alt="avatar" width={48} height={48} unoptimized className="w-full h-full object-cover" />
              </div>
            </div>
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-4 w-64 bg-white border border-[#D0D5CB] rounded-[24px] shadow-2xl py-3 z-[1001] animate-in fade-in zoom-in duration-200">
                <button onClick={handleProfileClick} className="w-full flex items-center gap-3 px-6 py-3 text-[14px] text-[#193C1F] hover:bg-[#F7F3ED] transition-colors">
                  <UserIcon /> My Profile
                </button>
                <div className="h-px bg-[#F7F3ED] my-2 mx-4" />
                <button 
                  onClick={() => { setIsProfileOpen(false); setShowLogoutAlert(true); }} 
