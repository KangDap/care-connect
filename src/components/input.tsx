...
export const Input = ({
  ...
  const inputStyles = 
    `${baseStyles} ${className} ${icon ? 'pl-14' : ''} ${isPassword ? 'pr-14' : ''}`;
    ...
    <div className="flex flex-col gap-2 w-full text-left">
      ...
      <div className="relative w-full flex items-center">
        {icon && (
          <div className="absolute left-5 text-[#8A087] z-10 pointer-events-none">
            {icon}
          </div>
        )}
      ...
      {isPassword && (
        <button
          ...
          className="absolute right-4 text-[#8A087] hover:text-[#193C1F] transition-colors focus:outline-none"
        >
          {showPassword ? (
